	...
	if ( is_array($fields ) ) {
		if ( isset($fields['blog_id']) ) {
			...
			if ( substr( $fields['domain'], 0, 4 ) == 'www.' ) {
				...
				$blog = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->blogs WHERE domain IN (%s,%s) AND path = %s ORDER BY CHAR_LENGTH(domain) DESC", strtolower( $nowww ), strtolower( $fields['domain'] ), strtolower( $fields['path'] ) ) );
			} else {
				$blog = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->blogs WHERE domain = %s AND path = %s", strtolower( $fields['domain'] ), strtolower( $fields['path'] ) ) );
			}
			if ( $blog ) {
			...
			if ( substr( $fields['domain'], 0, 4 ) == 'www.' ) {
				$nowww = substr( $fields['domain'], 4 );
				$blog = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->blogs WHERE domain IN (%s,%s) ORDER BY CHAR_LENGTH(domain) DESC", strtolower( $nowww ), strtolower( $fields['domain'] ) ) );
			} else {
				$blog = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->blogs WHERE domain = %s", strtolower( $fields['domain'] ) ) );
			}
			if ( $blog ) {
				wp_cache_set($blog->blog_id . 'short', $blog, 'blog-details');
				$blog_id = $blog->blog_id;
